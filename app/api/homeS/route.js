...
export async function GET(req) {
  try {
    const categories = await prisma.homeS.findMany();
...
export async function POST(req) {
  try {
    ...
    const category = await prisma.homeS.create({ data: { name,desc, img ,link } });
...
export async function PATCH(req) {
  ...
  try {
    ...
    const updatedCategory = await prisma.homeS.update({
      where: { id },
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.homeS.delete({ where: { id } });
