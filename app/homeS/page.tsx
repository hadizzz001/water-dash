...
const ManageCategory = () => {
  ...
  const fetchCategories = async () => {
    try {
      const res = await fetch('/api/homeS', { method: 'GET' });
      if (res.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.ok) {
      setMessage('home service added successfully!');
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/homeS?id=${encodeURIComponent(editFormData.id)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleDelete = async (id) => {
    if (confirm(`Are you sure you want to delete this home home service?`)) {
      try {
        const res = await fetch(`/api/homeS?id=${encodeURIComponent(id)}`, {
          method: 'DELETE',
        });
        if (res.ok) {
          setMessage('Home home service deleted successfully!');
          fetchCategories();
          redirect('/homeS');
        } else {
          const errorData = await res.json();
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">{editMode ? 'Edit home service' : 'Add home service'}</h1>
      ...
      />
        ...
        <button type="submit" className="bg-blue-500 text-white px-4 py-2">
          {editMode ? 'Update home service' : 'Add home service'}
        </button>
      </form>
      {message && <p className="mt-4">{message}</p>}

      <h2 className="text-xl font-bold mt-8">All home services</h2>
      <table className="table-auto border-collapse border border-gray-300 w-full mt-4">
        ...
        <tbody>
          {categories.length > 0 ? (
            ...
            <tr>
              <td className="border border-gray-300 p-2 text-center">No home services found.</td>
            </tr>
          )}
        </tbody>
      </table>
